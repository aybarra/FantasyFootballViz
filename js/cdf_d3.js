function generateCDF_D3Chart( data )
{
    ...
    x.domain( [
        0,
        d3.max( data_conv, function ( d )
        {
            return d.values.length;
        } )  // Want the longest length career
    ] );

    xTime.domain( d3.extent( data_conv, function ( d )
    {
        return d.year;
    } ) );

    y.domain( [
        d3.min( data_conv, function ( d )
        {
    ...
    player.append( "text" )
        ...
        {
            return "translate(" + x( d_sub.value.x ) + "," + y( d_sub.value.y ) + ")";
        } )
        .attr( "x", 3 )
        .attr( "dy", ".35em" )
        .text( function ( d_sub )
        {
            return d_sub.name + ' (' + d_sub.value.y + ') ';
        } )
        .attr( "opacity", "0" );
    ...
    if( data !== undefined && data.length > 0 )
    {
        ...
        for ( var item in data )
        {
            ...
            if( !(pguid in lines) )
            {
                ...
                lines[ pguid ][ 'values' ].push( {
                    x: 1,
                    y: ff_pts,
                    year: d3.time.format( "%Y" ).parse( season_year )
                } );
